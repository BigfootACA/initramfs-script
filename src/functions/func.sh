...
function on_failed(){
	...
	case "${init_failed}" in
		sh|shell|bash)
			echo "drop into shell..."
			sync
			run_tty /dev/console /usr/bin/bash
			sleep 1
			sync
		;;
		reboot|reset|restart|reload)
			echo "rebooting..."
			sync
			sleep 1
			reboot -f
			exit 1
		;;
		halt|shutdown|poweroff|powerdown)
			echo "shutdowning..."
			sync
			sleep 1
			poweroff -f
